...
import type { EmployeeResponse, EmployeeWithRoles, RoleResponse, AssignRoleRequest } from "../../types/rolePermissionType";
...
export default function AdminEmployeePage() {
    ...
    const loadEmployees = async () => {
        try {
            setLoading(true);
            setError(null);
            const data = await employeeRoleService.getAllEmployeesAsync();
            setEmployees(data);
        } catch (error: unknown) {
            logError('AdminEmployeePage.loadEmployees', error);
            setError(extractErrorMessage(error, 'Failed to load employees data'));
        } finally {
            setLoading(false);
        }
    };
    ...
    const loadRoles = async () => {
        try {
            ...
            logError('AdminEmployeePage.loadRoles', error);
    ...
    const handleOpenAssignDialog = async (employee: EmployeeResponse) => {
        ...
        try {
            ...
            const employeeData = await employeeRoleService.getEmployeeWithRolesAsync(employee.id);
            const currentRoleIds = employeeData.roles?.map(r => r.id) || [];
            setSelectedRoleIds(currentRoleIds);
        } catch (error: unknown) {
            logError('AdminEmployeePage.handleOpenAssignDialog', error);
    ...
    const handleAssignRoles = async () => {
        ...
        try {
            ...
            const result = await employeeRoleService.assignRolesToEmployeeAsync(request);
            ...
            logError('AdminEmployeePage.handleAssignRoles', error);
            showSnackbar(extractErrorMessage(error, 'Error assigning roles'), "error");
        } finally {
            setAssignDialogLoading(false);
        }
    };
    ...
    const handleOpenDetailDialog = async (employee: EmployeeResponse) => {
        ...
        try {
            ...
            const data = await employeeRoleService.getEmployeeWithRolesAsync(employee.id);
            setEmployeeDetails(data);
        } catch (error: unknown) {
            logError('AdminEmployeePage.handleOpenDetailDialog', error);
            showSnackbar(extractErrorMessage(error, 'Failed to load employee details'), "error");
