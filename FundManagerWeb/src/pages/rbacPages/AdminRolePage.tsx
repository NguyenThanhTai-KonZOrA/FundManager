...
import type { RoleResponse, PermissionResponse, CreateRoleRequest, UpdateRoleRequest } from "../../types/rolePermissionType";
...
export default function AdminRolePage() {
    ...
    const loadRoles = async () => {
        try {
            setLoading(true);
            setError(null);
            const data = await roleService.getAllRolesAsync();
            setRoles(data);
        } catch (error: unknown) {
            logError('AdminRolePage.loadRoles', error);
            setError(extractErrorMessage(error, 'Failed to load roles data'));
        } finally {
            setLoading(false);
        }
    };
    ...
    const loadPermissions = async () => {
        try {
            const data = await permissionService.getAllPermissionsAsync();
            setPermissions(data.filter(p => p.isActive)); // Only show active permissions
        } catch (error: unknown) {
            logError('AdminRolePage.loadPermissions', error);
            showSnackbar(extractErrorMessage(error, 'Failed to load permissions'), "error");
        }
    };
    ...
    const handleToggleStatus = async (role: RoleResponse) => {
        try {
            ...
            const result = await roleService.changeStatusAsync(role.id);
            ...
            logError('AdminRolePage.handleToggleStatus', error);
            showSnackbar(extractErrorMessage(error, 'Error updating role status'), "error");
        } finally {
    ...
    const handleOpenEditDialog = async (role: RoleResponse) => {
        ...
        try {
            ...
            const roleDetails = await roleService.getRoleByIdAsync(role.id);
            ...
            logError('AdminRolePage.handleOpenEditDialog', error);
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (dialogMode === "create") {
                ...
                const result = await roleService.createRoleAsync(request);
                ...
                const result = await roleService.updateRoleAsync(editingRole.id, request);
                setRoles(prev =>
            ...
            logError('AdminRolePage.handleSubmit', error);
            const fallbackMessage = dialogMode === "create"
                ? "Error creating role"
                : "Error updating role";
            showSnackbar(extractErrorMessage(error, fallbackMessage), "error");
        } finally {
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            await roleService.deleteRoleAsync(deletingRole.id);
            setRoles(prev => prev.filter(r => r.id !== deletingRole.id));
            showSnackbar(`Role "${deletingRole.roleName}" deleted successfully!`, "success");
            handleCloseDeleteDialog();
        } catch (error: unknown) {
            logError('AdminRolePage.handleDelete', error);
            showSnackbar(extractErrorMessage(error, 'Error deleting role'), "error");
        } finally {
