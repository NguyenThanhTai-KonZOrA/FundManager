...
import { staffDeviceService } from './deviceMappingService';
...
interface StaffDeviceRegisteredMessage {
    success: boolean;
    message?: string;
...
interface SignalRConnectionInfo {
    ...
    baseUrl: string;
    staffDeviceId: number | null;
    staffDeviceName: string | null;
    isConnected: boolean;
...
type SignalREventCallback = (...args: unknown[]) => void;

declare global {
    interface Window {
        signalRDebug?: Record<string, unknown>;
    }
}
...
class SignalRService {
    ...
    private eventListeners: Map<string, SignalREventCallback[]> = new Map();
    ...
    private hubUrl = `${getApiBase()}/patronSignatureHub`;
    ...
    private setupEventHandlers(): void {
        ...
        this.connection.on('StaffDeviceRegistered', (response: StaffDeviceRegisteredMessage) => {
    ...
    private startOnlineDevicesCheck(): void {
        ...
        const checkOnlineDevices = async () => {
            try {
                ...
                const onlineDevices = await staffDeviceService.getOnlineStaffDevicesAsync();
    ...
    private storeEventListener(eventName: string, callback: SignalREventCallback): void {
    ...
    public onSignatureCompleted(callback: (message: SignatureCompletedMessage) => void): void {
        ...
        const wrappedCallback = (message: unknown) => {
            if (!message || typeof message !== 'object') {
                console.warn('⚠️ Invalid SignatureCompleted message payload:', message);
                return;
            }
            ...
            // console.log(`   SessionId: ${String(payload.sessionId ?? '')}`);
            // console.log(`   PatronId: ${String(payload.patronId ?? '')}`);
            // console.log(`   FullName: ${String(payload.fullName ?? '')}`);
            // console.log(`   MobilePhone: ${String(payload.mobilePhone ?? 'N/A')}`);
            // console.log('============================================');
            // console.log('');

            const patronIdValue = Number(payload.patronId);
            const sessionIdRaw = payload.sessionId;
            const sessionIdString = typeof sessionIdRaw === 'string' || typeof sessionIdRaw === 'number'
                ? String(sessionIdRaw)
                : '';
            const fullName = typeof payload.fullName === 'string' ? payload.fullName : '';
            const mobilePhone = typeof payload.mobilePhone === 'string' ? payload.mobilePhone : undefined;

            if (!Number.isFinite(patronIdValue) || !sessionIdString || !fullName) {
                console.warn('⚠️ SignatureCompleted payload missing required fields:', payload);
                return;
            }
            ...
            const transformedMessage: SignatureCompletedMessage = {
                ...
                fullName,
                mobilePhone,
                sessionIdNumber: typeof sessionIdRaw === 'number' ? sessionIdRaw : parseInt(sessionIdString)
            };
    ...
    public onNewRegistration(callback: (message: NewRegistrationMessage) => void): void {
        ...
        const wrappedCallback = (message: unknown) => {
            if (!message || typeof message !== 'object') {
                console.warn('⚠️ Invalid NewRegistration message payload:', message);
                return;
            }
            const payload = message as Record<string, unknown>;

            const transformedMessage: NewRegistrationMessage = {
                patronId: Number(payload.patronId),
                fullName: typeof payload.fullName === 'string' ? payload.fullName : '',
                submitType: Number(payload.submitType)
            };

            if (!Number.isFinite(transformedMessage.patronId) || !transformedMessage.fullName || !Number.isFinite(transformedMessage.submitType)) {
                console.warn('⚠️ NewRegistration payload missing required fields:', payload);
                return;
            }
    ...
    // Get connection info
    public getConnectionInfo(): SignalRConnectionInfo {
        if (!this.connection) {
            return {
                state: 'Disconnected',
                connectionId: null,
