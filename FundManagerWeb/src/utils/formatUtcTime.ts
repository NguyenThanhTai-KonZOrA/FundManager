export class FormatUtcTime {
  ...
  // Format date to dd/MM/yyyy format (e.g., 26/12/2025 12:30:45)
  static formatDateDDMMMYYYYV2(dateStr?: string | Date): string {
    if (!dateStr) return "";
  ...
  static formatDateDDMMAndTime(dateStr?: string | Date): string {
    if (!dateStr) return "";
  ...
  // Format MMM dd, yyyy format (e.g., Dec 26, 2025)
  static formatDateMMMDDYYYY(dateStr?: string | Date): string {
    if (!dateStr) return "";
    let d: Date;
    if (typeof dateStr === 'string') {
      if (dateStr === "0001-01-01T00:00:00") return "-";
